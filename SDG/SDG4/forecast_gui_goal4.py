...
class ForecastAppGoal4:
    def __init__(self, root):
        self.root = root
        self.root.title("SDG 4 Indicator Forecast")
    ...
    def load_data(self):
        ...
        try:
            file_path = os.path.join(self.current_dir, 'Goal4_processed.csv')
    ...
    def create_selection_widgets(self):
        ...
        # Gender selection
        ttk.Label(self.selection_frame, text="Gender:").grid(row=3, column=0, padx=2, pady=2, sticky=tk.W)
        ...
        self.gender_combo.grid(row=3, column=1, padx=2, pady=2, sticky=tk.W)
        
        # Right column
        # Education Level selection
        ttk.Label(self.selection_frame, text="Education:").grid(row=0, column=2, padx=2, pady=2, sticky=tk.W)
        ...
        self.education_combo.grid(row=0, column=3, padx=2, pady=2, sticky=tk.W)
        
        # Type of Skill selection
        ttk.Label(self.selection_frame, text="Skill:").grid(row=1, column=2, padx=2, pady=2, sticky=tk.W)
        self.skill_var = tk.StringVar()
        self.skill_combo = ttk.Combobox(self.selection_frame, textvariable=self.skill_var, width=15)
        self.skill_combo['values'] = [
            'ALL',
            ...
            'SOFT (Software)',
            'EMAIL (Email)',
            'ONLSFT (Online Software)',
            'INST (Internet)',
            'SCRTY (Security)',
            'CMFL (Computer File)',
            'DLDONLD (Download)',
            'VOIP (Voice over IP)',
            'GSINF (Geographic Information)',
            'PRVCY (Privacy)',
            'SNTWK (Social Network)',
            'HLTHINF (Health Information)',
            'EPRS (Electronic Presentation)',
            'COPA (Copy/Paste)',
            'PCPR (Personal Computer)',
            'TRAF (Traffic)',
            'UPLD (Upload)',
            'INTBNK (Internet Banking)',
            'FONLCRS (Free Online Course)',
            'ONLCNS (Online Consultation)',
            'VRFY (Verify)',
            'ARSP (Arithmetic Spreadsheet)',
            'GSPUR (Government Service Purchase)',
            'ONLCRS (Online Course)'
        ]
        self.skill_combo.set('ALL')
        self.skill_combo.grid(row=1, column=3, padx=2, pady=2, sticky=tk.W)
        
        # Age selection
        ttk.Label(self.selection_frame, text="Age:").grid(row=2, column=2, padx=2, pady=2, sticky=tk.W)
        ...
        self.age_combo.grid(row=2, column=3, padx=2, pady=2, sticky=tk.W)
        
        # Location selection
        ttk.Label(self.selection_frame, text="Location:").grid(row=3, column=2, padx=2, pady=2, sticky=tk.W)
        ...
        self.location_combo.grid(row=3, column=3, padx=2, pady=2, sticky=tk.W)
        ...
        self.forecast_button.grid(row=4, column=0, columnspan=4, padx=5, pady=5)
    ...
    def show_immediate_data_assessment(self):
        ...
        try:
            ...
            self.results_text.insert(tk.END, f"🎯 DATA QUALITY ASSESSMENT (SDG4)\n")
            ...
            if len(indicator_data) > 0:
                ...
                if quality_score >= 80:
                    self.results_text.insert(tk.END, f"   ✅ EXCELLENT - Ideal for reliable education forecasting\n")
                elif quality_score >= 65:
                    self.results_text.insert(tk.END, f"   ✅ GOOD - Suitable for education policy forecasting\n")
            ...
            self.results_text.insert(tk.END, f"🔗 EXTERNAL VARIABLES AVAILABILITY (SDG4):\n")
            ...
            if external_available >= 4 and quality_score >= 70:
                ...
                self.results_text.insert(tk.END, f"      → Rich external education data + excellent historical data\n")
                self.results_text.insert(tk.END, f"      → Best for complex education indicator relationships\n")
                ...
                self.results_text.insert(tk.END, f"      → Suitable for education policy planning\n")
                ...
                self.results_text.insert(tk.END, f"      → Moderate confidence for education trends\n")
                ...
                self.results_text.insert(tk.END, f"      → Reliable for trend-based education forecasting\n")
            else:
                ...
                self.results_text.insert(tk.END, f"      → Use with caution for education planning\n")
            
            # Add SDG4-specific education context
            self.results_text.insert(tk.END, f"\n🎓 SDG4 EDUCATION CONTEXT:\n")
            if "4.1" in indicator_id:
                self.results_text.insert(tk.END, f"   📚 Primary/Secondary Education: Access & completion trends\n")
            elif "4.2" in indicator_id:
                self.results_text.insert(tk.END, f"   👶 Early Childhood Education: Development foundation\n")
            elif "4.3" in indicator_id:
                self.results_text.insert(tk.END, f"   🎓 Higher Education: Skills & employment preparation\n")
            elif "4.4" in indicator_id:
                self.results_text.insert(tk.END, f"   💻 Digital Skills: Technology literacy trends\n")
            elif "4.5" in indicator_id:
                self.results_text.insert(tk.END, f"   ⚖️ Education Equity: Gender & inclusion disparities\n")
            elif "4.6" in indicator_id:
                self.results_text.insert(tk.END, f"   📖 Adult Literacy: Lifelong learning needs\n")
            elif "4.7" in indicator_id:
                self.results_text.insert(tk.END, f"   🌍 Sustainable Development Education: Values & awareness\n")
            elif "4.a" in indicator_id:
                self.results_text.insert(tk.END, f"   🏫 Education Infrastructure: Facilities & resources\n")
            elif "4.b" in indicator_id:
                self.results_text.insert(tk.END, f"   🎒 Scholarships: Access & mobility support\n")
            elif "4.c" in indicator_id:
                self.results_text.insert(tk.END, f"   👨‍🏫 Teachers: Training & qualification status\n")
            
            self.results_text.insert(tk.END, f"\n" + "="*50 + "\n")
            self.results_text.insert(tk.END, f"💡 Ready to generate education forecast! Select model and click 'Generate Forecast'\n")
    ...
    def fit_random_forest_model(self, series, country):
        """Fit Enhanced Random Forest model"""
        try:
            print(f"\nFitting Enhanced Random Forest model for {country}")
    ...
    def fit_sarimax_model(self, series, country):
        ...
        if len(all_features_by_year) < 8:  # Use same absolute threshold as SDG2/SDG3
        ...
        # Align series with available external data (like SDG2/SDG3)
    ...
    def generate_forecast(self):
        ...
        try:
            ...
            education = self.education_var.get().split(' (')[0]  # Extract code part before parentheses
            ...
            indicator_data['TypeofSkill'] = indicator_data['TypeofSkill'].fillna('ALL')
            ...
            if skill != 'ALL':
                skill_code = skill.split(' (')[0]  # Extract code part before parentheses
                indicator_data = indicator_data[indicator_data['TypeofSkill'] == skill_code]
                if len(indicator_data) == 0:
                    messagebox.showerror("Error", f"No data found for type of skill {skill} in {indicator_id} for {country}")
                    return
            ...
            if len(indicator_data) < MIN_DATA_POINTS:
                messagebox.showerror("Error", 
                    f"Not enough data points for {gender}, education {education}, age {age}, and location {location}.\n"
                    f"Found {len(indicator_data)} points, but need at least {MIN_DATA_POINTS} points "
                    f"for a reliable forecast.\n"
                    f"Please try a different indicator, country, gender, education level, age group, or location for more data points.")
                return
            ...
            try:
                if model_type == 'ARIMA':
                    ...
                    # Calculate prediction intervals (wider than confidence intervals)
                    pred_interval = 2.0 * (rmse / scale_factor)  # Increased multiplier for more visible intervals
                    ...
                    conf_interval = 1.0 * (rmse / scale_factor)  # Smaller multiplier for confidence intervals
                    ...
                    scaled_forecast = forecast['yhat'].iloc[-5:].values / scale_factor
                    
                    # Get confidence intervals from Prophet and scale them
                    scaled_conf_lower_future = forecast['yhat_lower'].iloc[-5:].values / scale_factor
                    scaled_conf_upper_future = forecast['yhat_upper'].iloc[-5:].values / scale_factor
                    ...
                    if 'feature_names' in sarimax_results:
                        ...
                        # Scale the predictions and test data
                        scaled_predictions = test_predictions / scale_factor
                        scaled_test = test / scale_factor
                        
                        # Plot test predictions
                        prediction_color = plt.cm.Reds(0.7)
                        ax.scatter(test.index, scaled_predictions, color=prediction_color, 
                                  label='SARIMAX Test', s=100, alpha=0.8)
                        ax.plot(test.index, scaled_predictions, color=prediction_color, alpha=0.5, linewidth=2)
                        ...
                        future_exog = self.extrapolate_external_variables(
                            country, last_year, 5, sarimax_results['feature_names']
                        )
                        ...
                        future_forecast = model_fit.forecast(steps=5, exog=future_exog_scaled)
            ...
            if gender != 'BOTHSEX':
                title += f'\n{gender}'
            if education != 'ALL':
                title += f'\nEducation: {self.education_var.get()}'  # Show full name
            if skill != 'ALL':
                title += f'\nType of Skill: {self.skill_var.get()}'  # Show full name
            if age != 'ALLAGE':
            ...
            self.results_text.insert(tk.END, f"Education Level: {self.education_var.get()}\n")  # Show full name
            self.results_text.insert(tk.END, f"Type of Skill: {self.skill_var.get()}\n")  # Show full name
            ...
            try:
                ...
                if model_type == 'Random Forest' and 'rf_results' in locals() and 'feature_importance' in rf_results:
                    self.results_text.insert(tk.END, "\nFeature Importance (%):\n")
    ...
    def show_external_data_status(self):
        ...
        status_text = "\n=== External Data Integration Status (SDG 4) ===\n"
    ...
    def save_plot(self):
        ...
        if self.current_fig:
            ...
            default_filename = f"SDG4_{indicator_id}_{country}_{gender}_{education}_{age}_{location}.png"
    ...
    def load_external_data(self):
        ...
        try:
            ...
            sdg4_dir = os.path.dirname(current_file)  # SDG4 directory
            parent_dir = os.path.dirname(sdg4_dir)  # SDG parent directory
