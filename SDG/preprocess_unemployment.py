...
def process_Unemployment_data():
    # Read the Unemployment data, skipping the first 4 metadata rows
    # The file uses double quotes and commas in a specific format
    df = pd.read_csv('Unemployment.csv', skiprows=4, quotechar='"', skipinitialspace=True)
    
    print("Original Unemployment data shape:", df.shape)
    ...
    # Melt the dataframe to have Year and Unemployment columns
    df_melted = df.melt(
    ...
    # Convert Year to integer and Unemployment to numeric
    df_melted['Year'] = df_melted['Year'].astype(int)
    df_melted['Unemployment'] = pd.to_numeric(df_melted['Unemployment'], errors='coerce')
    
    # Remove rows with missing Unemployment values
    df_melted = df_melted.dropna(subset=['Unemployment'])
    ...
    df_melted.to_csv('Unemployment_processed.csv', index=False)
    print("Processed Unemployment data saved to Unemployment_processed.csv")
