...
def process_GINI_data():
    # Read the GINI data, skipping the first 4 metadata rows
    # The file uses double quotes and commas in a specific format
    df = pd.read_csv('GINI.csv', skiprows=4, quotechar='"', skipinitialspace=True)
    
    print("Original GINI data shape:", df.shape)
    ...
    # Melt the dataframe to have Year and GINI columns
    df_melted = df.melt(
    ...
    df_melted['Gini index'] = pd.to_numeric(df_melted['Gini index'], errors='coerce')
    
    # Remove rows with missing GDP values
    df_melted = df_melted.dropna(subset=['Gini index'])
    ...
    df_melted.to_csv('GINI_processed.csv', index=False)
    print("Processed GINI data saved to GINI_processed.csv")
