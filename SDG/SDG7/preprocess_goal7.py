...
def preprocess_data():
    try:
        ...
        input_file = os.path.join(parent_dir, 'Goal7.csv')
        output_file = os.path.join(current_dir, 'Goal7_processed.csv')
        ...
        print("Reading Goal7.csv...")
        df = pd.read_csv(input_file, sep=';', on_bad_lines='skip')
        
        # Check required columns
        required_columns = ['Indicator', 'SeriesCode', 'SeriesDescription', 'GeoAreaName', 'TimePeriod', 'Value', 'Source', 'Location', 'Type of renewable technology', 'Units']
        ...
        df = df.rename(columns={'Type of renewable technology': 'TypeofRenewableTechnology'})
