...
def process_ResearchAndDevelopment_data():
    # Read the Research and development expenditure data, skipping the first 4 metadata rows
    # The file uses double quotes and commas in a specific format
    df = pd.read_csv('R&D Expenditures.csv', skiprows=4, quotechar='"', skipinitialspace=True)
    
    print("Original R&D Expenditures data shape:", df.shape)
    ...
    if 'Country Name' not in df.columns:
        ...
        with open('R&D Expenditures.csv', 'r') as f:
            lines = f.readlines()
    ...
    # Melt the dataframe to have Year and GDP columns
    df_melted = df.melt(
    ...
    # Convert Year to integer and Research and development expenditure to numeric
    ...
    # Remove rows with missing Research and development expenditure values
    df_melted = df_melted.dropna(subset=['Research and development expenditure'])
    ...
    df_melted.to_csv('R&D Expenditures_processed.csv', index=False)
    print("Processed R&D Expenditures data saved to R&D Expenditures_processed.csv")
...
if __name__ == "__main__":
    process_ResearchAndDevelopment_data() 