...
def preprocess_data():
    try:
        ...
        input_file = os.path.join(parent_dir, 'Goal9.csv')
        output_file = os.path.join(current_dir, 'Goal9_processed.csv')
        ...
        print("Reading Goal9.csv...")
        df = pd.read_csv(input_file, sep=';', on_bad_lines='skip')
        
        # Check required columns
        required_columns = ['Indicator', 'SeriesCode', 'SeriesDescription', 'GeoAreaName', 'TimePeriod', 'Value', 'Source', 'Location','Units', 'Mode of transportation']
        ...
        df = df.rename(columns={'Mode of transportation': 'ModeofTransportation'})
