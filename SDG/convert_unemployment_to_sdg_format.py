#!/usr/bin/env python3
"""
Convert Unemployment.csv from wide format to SDG long format
...
def convert_unemployment_to_sdg_format():
    """Convert Unemployment.csv to SDG format like GDP_processed.csv"""
    print("🔧 Converting Unemployment.csv to SDG format...")
    
    try:
        # Read the wide-format unemployment file
        df = pd.read_csv('Unemployment.csv', sep=';')
        print(f"✅ Loaded unemployment data: {len(df)} rows, {len(df.columns)} columns")
        ...
        df_long['Unemployment'] = df_long['Unemployment'].astype(str).str.replace(',', '.', regex=False)
        ...
        print(f"📊 Non-null unemployment values: {df_long['Unemployment'].notna().sum()}")
        
        # Remove rows with missing values (but be less aggressive)
        df_clean = df_long.dropna(subset=['Year', 'Unemployment'])
        print(f"📊 After cleaning: {len(df_clean)} rows")
        
        if len(df_clean) < 100:
            print("⚠️  Very few rows! Let's check what's happening...")
            print("Sample unemployment values:")
            print(df_long['Unemployment'].head(20))
            print("Value counts:")
            print(df_long['Unemployment'].value_counts().head(10))
        ...
        df_sdg.to_csv('Unemployment_processed.csv', sep=';', index=False)
        
        print(f"✅ Saved as Unemployment_processed.csv (SDG format, semicolon-separated)")
        
        # Verify by reading it back
        test_df = pd.read_csv('Unemployment_processed.csv', sep=';')
        print(f"✅ Verification: {len(test_df)} rows, {len(test_df.columns)} columns")
        print(f"Columns: {list(test_df.columns)}")
        
    except Exception as e:
        print(f"❌ Error converting unemployment data: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    convert_unemployment_to_sdg_format() 