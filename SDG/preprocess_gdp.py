...
def process_gdp_data():
    # Read the GDP data, skipping the first 4 metadata rows
    # The file uses double quotes and commas in a specific format
    df = pd.read_csv('GDP.csv', skiprows=4, quotechar='"', skipinitialspace=True)
    
    print("Original GDP data shape:", df.shape)
    ...
    df_melted.to_csv('GDP_processed.csv', index=False)
    print("Processed GDP data saved to GDP_processed.csv")
